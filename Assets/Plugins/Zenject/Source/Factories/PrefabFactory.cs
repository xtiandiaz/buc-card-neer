...
namespace Zenject
{
        ...
        //where T : Component
    {
        ...
        public virtual T Create(UnityEngine.Object withType)
        {
            Assert.That(withType != null,
               "Null prefab given to factory create method when instantiating object with type '{0}'.", typeof(T));

            return _container.InstantiatePrefabForComponent<T>(withType);
        }
        ...
        //where T : Component
    {
        ...
        public virtual T Create(UnityEngine.Object prefab, P1 inParentView)
        {
            Assert.That(prefab != null,
               "Null prefab given to factory create method when instantiating object with type '{0}'.", typeof(T));

            return (T)_container.InstantiatePrefabForComponentExplicit(
                typeof(T), prefab, InjectUtil.CreateArgListExplicit(inParentView));
        }
    }
