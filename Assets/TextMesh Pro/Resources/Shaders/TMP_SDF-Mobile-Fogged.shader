...
Shader "TextMeshPro/Mobile/Distance Field with Custom Fog" {
...
SubShader {
	...
	Pass {
		...
		fixed4 PixShader(pixel_t input) : SV_Target
		{
			...
			half4 c = lerp(input.faceColor * saturate(d - input.param.w), _FogColorPDS, _FogIntensityPDS);

			#ifdef OUTLINE_ON
			c = lerp(lerp(input.outlineColor, input.faceColor, saturate(d - input.param.z)), _FogColorPDS, _FogIntensityPDS);
